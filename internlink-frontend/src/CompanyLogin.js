...
function CompanyLogin({ setCompany, goToSignup, goBack }) {
  ...
  const login = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("http://localhost:5000/company-login", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, password })
      });
      const json = await res.json();
      if (json.success && json.data) {
        addToast(`Welcome, ${json.data.CompanyName}!`, "success");
  ...
  return (
    <div className="auth-container">
      <div className="glass-panel auth-card">
        ...
        <p className="subtitle">Manage your internship postings</p>
        ...
        <form onSubmit={login}>
          <input className="input-field" placeholder="Work Email" type="email" value={email} onChange={e => setEmail(e.target.value)} />
        ...
        <p style={{ color: "var(--text-muted)", fontSize: "0.9rem" }}>
          New to InternLink? <span style={{ color: "var(--accent)", cursor: "pointer", fontWeight: "600" }} onClick={goToSignup}>Register Company</span>
        </p>
